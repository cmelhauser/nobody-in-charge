...
turning either finite audit into a universal claim. The 400-seed decay runs
(`research/decay_ordering.json`, `research/decay_reversal.json`) are the concrete case: with the
decay rate a fifth lower or more, referral loss leaves the larger group but is still worse on
existence and endpoint viability.
