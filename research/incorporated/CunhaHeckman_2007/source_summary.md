...
`CunhaHeckman` rather than `Cunha`, for two reasons. A second Cunha paper is held, and two directories may not share a leading token; and `Cunha` occurs in Chapters 12 and 13, where it would pair citations with whichever file held the token. The compound token occurs nowhere in prose and pairs nothing.
