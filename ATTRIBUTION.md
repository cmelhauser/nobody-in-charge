# Authorship, acknowledgment, and rights

## Project credit

*Nobody in Charge*, its manuscript, paper, appendix, reference primer, original model code,
analysis code, documentation, and project-created outputs are credited as follows:

- Human Author: **Christopher Melhauser** (`christopher.melhauser@gmail.com`)
- AI Writing Collaborator: **theonlymuffinbot** (`theonlymuffinbot@outlook.com`)

“theonlymuffinbot” is the project's label for AI-assisted writing collaboration using a mix of
Anthropic Claude Opus 5 and OpenAI GPT-5.6 Sol and Terra models. It is an acknowledgment of that
collaboration, not a legal author or rights holder.

Christopher Melhauser is the human author and legal rights contact for the original project
material. The original project material is dedicated to the public domain under The Unlicense.
The project requests, but does not require, the courtesy credit above; see `LICENSE` for the
dedication and its scope.

## Scope of this notice

This notice applies only to original project material. It does not claim ownership of Alcoholics
Anonymous material, third-party scholarship, quotations, source metadata supplied by others, or
any other third-party content. Such material remains subject to its own copyright, license, and
provenance record.

The repository does not redistribute the source documents catalogued under
`research/incorporated/` and `research/staged/`. Their records identify the applicable rights and
provenance; see `research/SOURCES.md` and the individual source directories.

For citation metadata, use `CITATION.cff`. For permissions, see `LICENSE`.
