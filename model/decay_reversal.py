"""Where the referral-versus-attraction ordering first reverses, at 400 paired seeds.

`research/decay_ordering.json` estimates the pure-attraction-loss minus referral-loss ordering on
final membership at the model's decay rate and at 25, 50 and 75 per cent lower, and finds the
membership ordering reversed at all three lower rates. The multi-level one-at-a-time screen
(`research/oat_full.json`, three common seeds per endpoint) still finds it strict at 10 per cent
lower. This design fills that gap at 400 paired seeds, so that Chapter 12 can say how much more
slowly a practice must lapse before the outcomes disagree. It changes no released number.
...
Decay levels: `delta0` changed by -10, -15 and -20 per cent. Seeds 0-399 are shared across all
nine cells, so every contrast is paired by common random numbers. Horizon 1,560 weeks, dt 0.5.
The design, the interventions and the outcome definitions are those of `decay_ordering.py`; only
the levels differ, so the two caches can be read as one series.

Results cached to `../research/decay_reversal.json`; resumable.

Run:  python3 decay_reversal.py [n_workers] [batch]
"""
...
OUT = os.path.abspath(os.path.join(HERE, '..', 'research', 'decay_reversal.json'))
LEV = [-10, -15, -20]
...
if __name__ == '__main__':
    ...
    out['meta'] = dict(schema_version=2, status='incomplete', model_sha256=model_hash,
                       script_sha256=script_hash, script='model/decay_reversal.py',
